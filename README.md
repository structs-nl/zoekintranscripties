...
During the prototype competition, we made a first version of a transcription viewer, tailored search index and trained our own Bert model on historical Dutch, with Google TPU's. The latter was a very interesing experiment with this type of model, but did not yield better results than an off-the-shelf model. So we did not win with the NER, but our search and user interface aspects where appreciated. It was decided to split the commission: we would develop the search index, the user interface and the linked data. The NER and a CMS for the transcriptions was comissioned to one of the other participants. We worked decoupled and used our own transcription import script, which can also be found in the repository.
